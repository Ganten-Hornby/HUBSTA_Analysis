...
class EvalCallBack(Callback):
    ...
    def plot_scatter(self, gathered_val_vis, gathered_val_label, trainer):
        ...
        num_classes = max(
            len(set(gathered_val_label)), 
            np.max(gathered_val_label)+1)
        colors = list(cc.glasbey)*1000000
        
        
        plt.scatter(
            gathered_val_vis[:, 0],
            gathered_val_vis[:, 1],
            c=[colors[label] for label in gathered_val_label], 
            s=s
        )
        return fig
        ...
        for val_index in range(num_val):
            ...
            if trainer.max_epochs == 1 and pl_module.visited or len(val_vis_current) > 0 and trainer.current_epoch > 1 and (trainer.current_epoch+1) % self.inter == 0:
                ...
                if not isinstance(val_vis_current[0], np.ndarray):
                    ...
                    gathered_val_label_all = trainer.strategy.all_gather(val_label).cpu().detach().numpy()
                else:
                ...
                if len(gathered_val_vis.shape) == 3:
                    gathered_val_input = gathered_val_input.reshape(-1, *gathered_val_vis.shape[2:])
                    ...
                    gathered_val_label_all = gathered_val_label_all.reshape(-1)
                
                if trainer.is_global_zero:          # 检查是否为 rank 0 的进程
                    # print('gathered_val_vis.shape', gathered_val_vis.shape)
                    # import pdb ; pdb.set_trace()
                    for feat_i in range(gathered_val_label_all.shape[1]):
                        try:
                            print('-')
                            gathered_val_label = gathered_val_label_all[:, feat_i].astype(np.int32)
                            ...
                            dict_log = {
                                dataset_name[val_index]+f"_svc{feat_i}": acc_mean, 
                                dataset_name[val_index]+f"_svc_rbf{feat_i}": acc_mean_rbf,
                                "epoch": trainer.current_epoch,
                                dataset_name[val_index]+f"_scatter{feat_i}": wandb.Image(fig_scatter),
                                dataset_name[val_index]+f"_fig_scatter_hyper{feat_i}": wandb.Image(fig_scatter_hyper),
                            ...
                            print(f'Error occurred in processing feature {feat_i}')
                    ...
                    # if hasattr(pl_module, 'p3') and pl_module.p3!=0 and val_index == 0 and trainer.max_epochs > 1:
                    #     save_path="save_model/p"+str(pl_module.p3)+"/"+pl_module.data_name
                    #     p3list=["_", "_","_"]
                    #     save_name=str(pl_module.p1)+str(pl_module.p2)+"_"+str(val_index)+".pth"
