        ...
        for val_index in range(num_val):
            ...
            if trainer.max_epochs == 1 and pl_module.visited or len(val_vis_current) > 0 and trainer.current_epoch > 1 and (trainer.current_epoch+1) % self.inter == 0:
                ...
                if trainer.is_global_zero:          # 检查是否为 rank 0 的进程
                    ...
                    dict_log = {
                        dataset_name[val_index]+" svc": acc_mean, 
                        dataset_name[val_index]+' svc high': acc_mean_high, 

                        "epoch": trainer.current_epoch,
                        dataset_name[val_index]+" scatter": wandb.Image(fig_scatter),
                        dataset_name[val_index]+" fig_scatter_hyper": wandb.Image(fig_scatter_hyper),
                        # 'fig_heatmap_weight': fig_heatmap_weight
                        } 
                    if self.fully_eval:
                        ecb_e_train = ecb.Eval(input=gathered_hight_vis, latent=gathered_val_vis, label=gathered_val_label, k=10)
                        acc_kmeans=ecb_e_train.E_Clasting_Kmeans()
                        ZX,XZ=ecb_e_train.E_mrre()
                        acc_rre=float(float(ZX)+float(XZ))/2                    
                        dict_log[dataset_name[val_index]+" kmeans"]=acc_kmeans
                        dict_log[dataset_name[val_index]+" mrre"]=acc_rre
                    ...
                    if hasattr(pl_module, 'p3') and pl_module.p3!=0 and val_index == 0 and trainer.max_epochs > 1:
                        ...
                        p3list=["_", "_","_"]
                        save_name=str(pl_module.p1)+str(pl_module.p2)+p3list[pl_module.p3]+str(val_index)+".pth"
