        ...
        for val_index in range(num_val):
            ...
            if trainer.max_epochs == 1 and pl_module.visited or len(val_vis_current) > 0 and trainer.current_epoch > 1 and (trainer.current_epoch+1) % self.inter == 0:
                ...
                if trainer.is_global_zero:          # 检查是否为 rank 0 的进程
                    ...
                    fig_scatter = self.plot_scatter(gathered_val_vis, gathered_val_label.flatten(), trainer)
                    fig_scatter_hyper = self.plot_scatter_hyper(gathered_val_vis, gathered_val_label.flatten(), trainer)
                    
                    # os.makedirs('saveoutput', exist_ok=True)
                    # np.save(f'saveoutput/save_res_emb{val_name}.npy', gathered_val_vis)
                    # np.save(f'saveoutput/save_res_label{val_name}.npy', gathered_val_label)
                    # np.save(f'saveoutput/save_res_high{val_name}.npy', gathered_hight_vis)
                    # np.save('save_res_index', gathered_val_label)
