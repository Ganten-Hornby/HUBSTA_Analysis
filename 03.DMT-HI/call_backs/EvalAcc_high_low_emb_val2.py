...
class EvalCallBack(Callback):
    ...
    def plot_scatter_plotly(self, gathered_val_vis, gathered_val_label, trainer):
        # Convert the input arrays to a pandas DataFrame for easier handling
        df = pd.DataFrame({
            'x': gathered_val_vis[:, 0],
            'y': gathered_val_vis[:, 1],
            'label': gathered_val_label
        })
        
        # Define marker size based on the condition
        marker_size = 1 if df.shape[0] >= 10000 else 3
        ...
        fig = px.scatter(
            ...
            y='y', 
            color='label', 
            color_continuous_scale='rainbow',  # You can adjust the color scale
            title="Scatter Plot",
        )
        ...
        for val_index in range(num_val):
            ...
            if True:
                ...
                if trainer.is_global_zero:          # 检查是否为 rank 0 的进程
                    ...
                    fig_scatter = self.plot_scatter_plotly(gathered_val_vis, gathered_val_label, trainer)
                    ...
                    dict_log = {
                        ...
                        dataset_name[val_index]+"_scatter": fig_scatter,
