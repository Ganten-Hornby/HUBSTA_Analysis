...
class DigitsDataset(data.Dataset):
    def __init__(
        ...
        data, label, index_list = self.load_data(data_path=data_path)
        ...
        if train_val == 'train':
            ...
            self.ori_index = train_index
            np.save('saveoutput/train_ori_index.npy', train_index)
        elif train_val == 'val':
            self.data = val_data
            self.label = val_label
            self.ori_index = valid_index
            np.save('saveoutput/vali_ori_index.npy', valid_index)
        elif train_val == 'test':
            self.data = test_data
            self.label = test_label
            self.ori_index = test_index
            np.save('saveoutput/test_ori_index.npy', test_index)
    ...
    def split_data(self, data, label, index_ori, split_ratio=0.8, split_ratio_test=0.1, train_val='train', data_name='',index_save_path='data', index_save_name='train_val.npy'):
        ...
        train_index = index_ori[train_index]
        valid_index = index_ori[val_index]
        test_index = index_ori[test_index]
    ...
    def __len__(self):
        if self.data_name == 'Cifar10' or self.data_name == 'Cifar100':
            ...
            print('Cifar10 or Cifar100, return self.data.shape[0] * 10')
        else:
            return self.data.shape[0]
            print('not Cifar10 or Cifar100, return self.data.shape[0]')
