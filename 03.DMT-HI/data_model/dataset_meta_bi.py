...
class DigitsDataset(data.Dataset):
    def __init__(
        ...
        if use_neighbor:
            self.neighbors_index = self.Preprocessing(
                data, k, pca_dim, uselabel, train_val, split_ratio
            )
        
    def split_data(self, data, label, split_ratio=0.8, train_val='train', data_name='',index_save_path='data', index_save_name='train_val.npy'):
        ...
        index_save_name_train = f'trainval_index_train_{split_ratio}_{data_name}.npy'
        index_save_name_val = f'trainval_index_val_{split_ratio}_{data_name}.npy'
        if not os.path.exists(os.path.join(index_save_path, index_save_name_train)):
            print('save index', index_save_name_train, index_save_name_val)
            rand_index = np.random.permutation(data_len)
            train_index = rand_index[:train_len]
            val_index = rand_index[train_len:]
            ...
            print('load index', index_save_name_train, index_save_name_val)
    ...
    def Preprocessing(self, data, k, pca_dim, uselabel, token_index=0, train_val='train', split_ratio=0.8):
