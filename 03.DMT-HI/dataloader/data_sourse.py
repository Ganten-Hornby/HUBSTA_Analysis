...
class DigitsDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if "Cifar" in self.data_name:
            ...
            data_input_aug = data_input_item * alpha + data_input_aug_fro * (1 - alpha)
            label = self.label[index]
...
class DigitsRankSeqDataset(data.Dataset):
    def __init__(
        self,
        data_name="Digits",
        train=True,
        ...
        if "Cifar" not in self.data_name and preprocess_bool:
            neighbors_index = self.Preprocessing(
                data=data.reshape(data.shape[0], -1),
                k=k,
                pca_dim=pca_dim,
                uselabel=uselabel,
    ...
    def cal_near_index(self, k=10, device="cuda", uselabel=False, pca_dim=100):
        filename = (
            "save_near_index/data_name{}K{}uselabel{}pcadim{}_train{}.pkl".format(
                self.data_name, k, uselabel, pca_dim, self.train
            )
        )
        os.makedirs("save_near_index", exist_ok=True)
        # import pdb; pdb.set_trace()
        if not os.path.exists(filename):
            ...
            if not uselabel:
                ...
                for i in range(neighbors_index.shape[0]):
                    neighbor_index_i = neighbors_index[i]
                    not_same_label_mask = self.label[neighbor_index_i] != self.label[i]
                    neighbor_index_i[not_same_label_mask] = i
                    if not_same_label_mask.sum() > 0:
                        neighbors_index[i] = neighbor_index_i
                        print("find same label")
    ...
    def __getitem__(self, index):
        if self.train:
            if "Cifar" in self.data_name:
                ...
                rank_input = (np.argsort(data_input_item)[::-1]).copy()[
                    : int(self.sample_len * 1.1)
                ]
                rank_aug = (np.argsort(data_input_aug)[::-1]).copy()[
                    : int(self.sample_len * 1.1)
                ]

                data_input_input = data_input_item[rank_input]
                data_input_aug = data_input_aug[rank_input]
                # sample the rank
                sample_index_1 = np.random.choice(
                    rank_input.shape[0], self.sample_len, replace=False
                )
                sample_index_2 = np.random.choice(
                    rank_input.shape[0], self.sample_len, replace=False
                )
                mask_1 = np.zeros(rank_input.shape[0])
                mask_2 = np.zeros(rank_input.shape[0])
                mask_1[sample_index_1] = 1
                mask_2[sample_index_2] = 1
                rank_input = rank_input[mask_1 == 1]
                rank_aug = rank_aug[mask_2 == 1]
                label = self.label[index]
            ...
            return (
                (
                    rank_input,
                    rank_aug,
                    data_input_input[mask_1 == 1],
                    data_input_aug[mask_2 == 1],
                    data_input_item,
                ),
                label,
                index,
            ...
            return rank_input, data_input_item, label, index, self.token_index
