...
class TransformerEncoder(nn.Module):
    ...
    def network_single(self, num_input_dim, hidden_size, num_layers, nn_type='nn'):
        enc_list = []
        enc_list.append(NN_FCBNRL_MM(num_input_dim, hidden_size))  # 第一层
        for i in range(num_layers):
            enc_list.append(NN_FCBNRL_MM(hidden_size, hidden_size))  # 中间层
        enc_list.append(NN_FCBNRL_MM(hidden_size, hidden_size, use_RL=False))  # 最后一层
        enc = nn.ModuleList(enc_list)  # 用ModuleList保存每一层
        return enc
    
    def forward(self, input_x):
        all_embs = []  # 保存每一层的emb
        
        emb_layer = input_x  # 输入数据初始化为input_x
        
        # 逐层计算表征并保存
        for layer in self.enc:
    
            emb_layer = layer(emb_layer)
            all_embs.append(emb_layer)  # 保存每一层的表征

        emb = self.fc(emb_layer)  # 最后一层的表征经过全连接层得到最终的输出

        return emb, all_embs  # 返回最终的emb和每层的表征
...
class DMTEVT_model(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.vis = self.InitNetworkMLP(NS=[num_input_dim, 784, vis_dim], last_relu=False)
    ...
    def forward(self, x, tau=100.0):
        ...
        #     weight = x_masked
        x_masked = x
        lat_higt_dim_out, emb_list = self.enc(x_masked)
        ...
        # print('lat_higt_dim_out', lat_higt_dim_out.shape)
        ...
        return x_masked, lat_higt_dim_out, lat_vis, lat_high_dim, emb_list
    ...
    def distance_preserving_loss(self, x, x_aug, z, z_aug):
        distance_x_x_aug = torch.nn.functional.pairwise_distance(x, x_aug, p=2)
        distance_z_z_aug = torch.nn.functional.pairwise_distance(z, z_aug, p=2)
        loss = torch.nn.functional.mse_loss(distance_x_x_aug, distance_z_z_aug)
        return loss
    ...
    def training_step(self, batch, batch_idx):
        ...
        x_masked, lat_high_dim_exp, lat_vis, _, emb_list = self(
            data_input_item, 
        ...
        batch_size = data_input_item.shape[0] // 2
        dp_loss_list = []
        for i in range(len(emb_list)-1):
            dp_x = emb_list[i][:batch_size]
            dp_x_aug = emb_list[i][batch_size:]
            dp_z = emb_list[i+1][:batch_size]
            dp_z_aug = emb_list[i+1][batch_size:]
        
            dp_loss = self.distance_preserving_loss(
                dp_x, dp_x_aug, dp_z, dp_z_aug
            )
            dp_loss_list.append(dp_loss)
            
        dp_loss_mean = torch.stack(dp_loss_list).mean()
        
        # loss_all = (loss_emb + loss_lat) / 2 + orthogonal_loss*10#+ loss_mse*self.hparams.weight_mse # + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
        loss_all = (loss_emb + loss_lat) / 2 + dp_loss_mean / 10# + orthogonal_loss*10#+ loss_mse*self.hparams.weight_mse # + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
        ...
        self.log('dp_loss_mean', dp_loss_mean, on_step=False, on_epoch=True, prog_bar=False, logger=True, sync_dist=True)
    ...
    def validation_step(self, batch, batch_idx, test=False, dataloader_idx=0):
        ...
        if dataloader_idx == 0:
            ...
            x_masked, lat_high_dim_exp, lat_vis, lat_high_dim, emb_list = self(
                data_input_item,
                tau=self.hparams.tau,
                )
