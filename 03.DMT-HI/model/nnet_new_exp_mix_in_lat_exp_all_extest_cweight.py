...
class DMTEVT_model(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.trainer.max_epochs > 1:
            weight_tail = float(self.current_epoch) / float(self.trainer.max_epochs-1) * self.hparams.weight_tail_end + (1 - float(self.current_epoch) / float(self.trainer.max_epochs-1) ) * self.hparams.weight_tail_begin
        else:
            weight_tail = self.hparams.weight_tail_begin
        # loss_all = (loss_emb + loss_lat) / 2 + orthogonal_loss*10#+ loss_mse*self.hparams.weight_mse # + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
        loss_all = ((1.0-weight_tail)*loss_emb + weight_tail*loss_lat) + orthogonal_loss*10#+ loss_mse*self.hparams.weight_mse # + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
