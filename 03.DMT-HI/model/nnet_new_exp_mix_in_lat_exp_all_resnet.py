...
class DMTEVT_model(LightningModule):
    ...
    def forward(self, x, tau=100.0):
        ...
        if len(x.shape) == 2:
            ...
            self.mask = self.mask.expand(x_shape, -1, -1)
            if self.mask.shape[0] != x_shape:
            ...
            if len(x.shape) == 2:
                ...
                x_masked = torch.einsum('bik,bk->bik', self.mask, x)
                x_masked = x_masked.reshape((x_shape, self.hparams.num_use_moe, num_channel, num_x, num_y))
        else:
            # import pdb; pdb.set_trace()
        ...
        lat_high_dim = lat_higt_dim_out.reshape((x_shape, -1))
