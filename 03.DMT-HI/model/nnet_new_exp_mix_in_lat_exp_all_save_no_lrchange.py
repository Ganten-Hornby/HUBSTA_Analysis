        ...
        **kwargs,
        ...
        if self.p3!=0 :
            if self.p3==1:
                ...
                p2_list = [0.1,0.3,0.5,0.7,0.9]
                self.hparams.nu_lat=p1_list[p1]
                self.hparams.nu_emb=p2_list[p2]
            if self.p3==2:
                p1_list = [1.5,2.0,3.0,4.0]
                p2_list = [0.00008,0.0008,0.008,0.08,0.8]
                self.hparams.exaggeration_lat=p1_list[p1]
                self.hparams.exaggeration_emb=p1_list[p1]
                self.hparams.nu_lat=p2_list[p2]
                self.hparams.nu_emb=p2_list[p2]
            if self.p3==3:
                p1_list = [1.5,2.0,3.0,4.0]
                p2_list = [0.000625,0.0025,0.01,0.04,0.16]
                self.hparams.exaggeration_lat=p1_list[p1]
                self.hparams.exaggeration_emb=p1_list[p1]
                self.hparams.nu_lat=p2_list[p2]
                self.hparams.nu_emb=p2_list[p2]
            if self.p3==5:
                p1_list = [1.5,2.0,3.0,4.0]
                p2_list = [0.02,0.04,0.08,0.16,0.32]
                self.hparams.exaggeration_lat=p1_list[p1]
                self.hparams.exaggeration_emb=p1_list[p1]
                self.hparams.nu_lat=p2_list[p2]
                self.hparams.nu_emb=p2_list[p2]
...
#            self.hparams.nu_emb=self.hparams.nu_lat
        ...
        if save_model :
            self.visited = True
            save_path="save_model/p"+str(self.p3)+"/"+self.data_name+"/"
            save_name=str(self.p1)+str(self.p2)+"_"+"0.pth"
            print('load ckpt from:', save_path)
            # if not os.path.exists(ckpt_path+'/all.ckpt'):
            #     convert_zero_checkpoint_to_fp32_state_dict(ckpt_path, ckpt_path+'/all.ckpt')
            # self.load_state_dict(torch.load(ckpt_path)['state_dict'])
            self.load_state_dict(torch.load(save_path+save_name))
    ...
    def configure_optimizers(self):
        ...
        lrsched = CosineAnnealingSchedule(
            optimizer, n_epochs=self.hparams.max_epochs , warmup_epochs=5, final_lr=self.hparams.lr
        )
        return {
            "optimizer": optimizer,
            "lr_scheduler": {
