...
class TransformerEncoder(nn.Module):
    def __init__(
        ...
        if num_input_dim == 3072:
            self.enc = ResNet(BasicBlock, [2, 2, 2, 2], 3)
        else:
            enc_list = []
    ...
    def forward(self, x, mask=None):
        # x_masked = x.reshape(x.shape[0], 1, x.shape[1]).expand(
...
class DMTEVT_model(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.vis = self.InitNetworkMLP(NS=[trans_out_dim, 500, vis_dim], last_relu=False)
        self.exp = self.InitNetworkMLP(NS=[vis_dim+vis_dim, 500, 500, num_input_dim*num_muti_mask])
    ...
    def LossManifold(
        ...
        sim_aa = 1 / (torch.cdist(features_a, features_a) * temperature).square().add(1)
        sim_bb = 1 / (torch.cdist(features_b, features_b) * temperature).square().add(1)
        sim_ab = 1 / (torch.cdist(features_a, features_b) * temperature).square().add(1)
    ...
    def batch_patten_loss(self, feature_tra):
        ...
        for i in range(feature_tra.shape[0]):
            vectors = feature_tra[i]
        ...
        feature_per_group = feature_tra.shape[1]
        for i in range(feature_per_group):
            vectors = feature_tra[:,i,:]
            norms = vectors.norm(dim=1, keepdim=True)
            # norms = torch.where(norms == 0, torch.tensor(1.0, device=vectors.device), norms)  # 避免除以0
            norm_vectors = vectors / norms
        ...
        return loss_in/feature_tra.shape[0] - loss_cr/feature_per_group
    ...
    def forward(self, x, tau=100.0, cond=None):
        ...
        # if tau is not None:
        #     self.mask = torch.zeros_like(weight)==0
        # else:
        ...
        # x with shape [250, 3, 32, 32] -> [250, 32, 32, 3]
        # x = x.permute(0, 2, 3, 1)
        
        x_masked, lat_high_dim = self.enc(x)
        lat_vis = self.vis(lat_high_dim)
        
        # x_masked = x_masked[:x_masked.shape[0]//2]
        # mask_for_x = self.mask[:x_masked.shape[0]].bool()
        # output_mask_select = torch.masked_select(
        #     x_masked, mask_for_x
        #     ).reshape((x_masked.shape[0], x_masked.shape[1], -1))
        
        return lat_high_dim, lat_vis, None
    ...
    def training_step(self, batch, batch_idx):
        ...
        lat_high_dim, lat_vis, x_masked = self(
            data_input_item, 
        ...
        # lat_vis_normed = (lat_vis - lat_vis.mean(0).detach()) / lat_vis.std(0).detach()
        
        # orthogonal_loss = self.batch_patten_loss(
        #     x_masked.permute(1, 2, 0)
        #     )
        ...
        # loss_lat = 0 # self.LossManifold(latent_data=lat_high_dim.reshape(lat_high_dim.shape[0], -1))
        ...
        # data_input_item_rec = data_input_item.reshape(data_input_item.shape[0], -1)
        # loss_topo = (loss_topo_ne*self.hparams.weight_nepo + loss_topo_po)/(1+self.hparams.weight_nepo) # + loss_crossentropy
        # import pdb; pdb.set_trace()
        # loss_ce = nn.CrossEntropyLoss()(self.cehead(lat_vis_normed), torch.cat([label, label]))
        # loss_mse = ((self.dec(lat_vis_normed)-data_input_item_rec)**2)[:data_input_item.shape[0]//2].mean()
        # loss_all = loss_topo + loss_mse*self.hparams.weight_mse + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
        # loss_all = loss_topo + loss_mse*self.hparams.weight_mse # + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
        loss_all = (loss_emb + loss_lat) / 2 #+ loss_mse*self.hparams.weight_mse # + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
        # loss_all = loss_mse*self.hparams.weight_mse # + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
        # loss_all = loss_mse*self.hparams.weight_mse # + 0.1*orthogonal_loss#+ kloss# + mlm_loss / 10
    ...
    def validation_step(self, batch, batch_idx, test=False, dataloader_idx=0):
        ...
        # print('validation_step dataloader_idx', dataloader_idx)
        # if dataloader_idx == 0:
        data_input_item = batch['data_input_item']
        data_input_aug = batch['data_input_aug']
        index = batch['index']
    ...
    def test_step(self, batch, batch_idx):
        ...
        lat_high_dim, cond, x_masked = self(
            data_input_item,
