...
class TransformerEncoder(nn.Module):
    def __init__(
        ...
        enc_list = []
        enc_list.append(NN_FCBNRL_MM(3072, hidden_size))
...
class DMTEVT_model(LightningModule):
    ...
    def forward(self, x, tau=100.0, cond=None):
        ...
        if tau is not None:
            ...
            # x = x * self.mask
        x_masked, lat_high_dim = self.enc(x, mask=self.mask)
        ...
        return lat_high_dim, lat_vis, output_mask_select
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_topo_ne, loss_topo_po = self.LossManifold(
            ...
            latent_data=lat_vis_normed.reshape(lat_vis_normed.shape[0], -1),
            sigma=self.hparams.sigma,
            w_nb=self.hparams.w_nb,
            w_fp=self.hparams.w_fp,
            n_select_each_sample=self.hparams.n_neg_sample,
            v_latent=self.hparams.v_latent,
        )
