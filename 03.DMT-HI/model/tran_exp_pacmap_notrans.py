...
class DMTEVT_model(LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        if ckpt_path is not None:
            ...
            self.load_state_dict(torch.load(ckpt_path+'/all.ckpt')['state_dict'])
    ...
    def LossManifold(
        ...
        # normalize the data
        # latent_data = (latent_data - latent_data.mean(0)) / latent_data.std(0).detach()

        # latent_data_1 = latent_data[: input_data.shape[0] // 2]
        # latent_data_2 = latent_data[(input_data.shape[0] // 2):]
        
        NB = torch.pairwise_distance(latent_data, latent_data_aug, p=2, keepdim=False) 
        NB = NB + 1
        ...
        for shift in range(n_select_each_sample):
            ...
            latent_data_2_rand = latent_data_aug[index_neg]
    ...
    def forward(self, x, tau=100.0, cond=None):
        ...
        if tau is not None:
            ...
            mask = torch.gather(self.mask, 1, x)
        # import pdb; pdb.set_trace()
        lat_high_dim = self.enc(x.half()/784)
    ...
    def training_step(self, batch, batch_idx):
        ...
        data_input_aug = data_input_aug.view(-1, 784)
        ...
        lat_high_dim = lat_high_dim.view(-1, 4, lat_high_dim.shape[-1])
        lat_vis = lat_vis.view(-1, 4, lat_vis.shape[-1])
        # import pdb; pdb.set_trace()

        loss_topo = 0       
        for i in range(3):
            loss_topo_ne, loss_topo_po = self.LossManifold(
                latent_data=lat_vis[:,0,:],
                latent_data_aug=lat_vis[:,i+1,:],
            ...
            loss_topo += loss_topo_item
            
        
        # loss_topo = torch.stack(topo_loss_list).mean()
        loss_mse = ((self.dec(lat_high_dim[:, 0, :])-data_feature)**2).mean()
