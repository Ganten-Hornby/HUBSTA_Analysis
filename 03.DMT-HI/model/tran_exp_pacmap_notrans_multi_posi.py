...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, mask=None):
        # import pdb; pdb.set_trace()
        x = x.reshape(x.shape[0], 1, x.shape[1]).expand(x.shape[0], 20, x.shape[1])
        emb = self.enc(x)
        return self.fc(emb)
...
class DMTEVT_model(LightningModule):
    ...
    def LossManifold(
        ...
        NB = torch.pairwise_distance(latent_data, latent_data_aug, p=2, keepdim=False) + 1
        loss_NB = (NB/(10+NB)).mean()
        
        n_select_each_sample = 1
        loss_fp_list = []
        for shift in range(n_select_each_sample):
            index_neg = torch.cat([
                torch.range(1, latent_data_aug.shape[0]-1-shift),
                torch.range(0, shift)
                ]).to(latent_data_aug.device).int()
            latent_data_2_rand = latent_data[index_neg]
            FP = torch.pairwise_distance(latent_data, latent_data_2_rand, p=2, keepdim=False) + 1
    ...
    def training_step(self, batch, batch_idx):
        ...
        data_input_aug = data_input_aug.view(-1, self.hparams.num_input_dim)
        data_input_item = torch.cat([data_input_item, data_input_aug])

        cond_all = torch.cat([cond.detach().to(self.device) for i in range(self.nps+1)])
        lat_high_dim, lat_vis = self(data_input_item, tau=self.tau, cond=cond_all)
        
        lat_high_dim_list = []
        lat_vis_list = []
        for i in range(self.nps+1):
            lat_high_dim_list.append(lat_high_dim[i*index.shape[0]:(i+1)*index.shape[0]])
            lat_vis_list.append(lat_vis[i*index.shape[0]:(i+1)*index.shape[0]])
        
        # lat_high_dim = lat_high_dim.view(-1, self.nps+1, lat_high_dim.shape[-1])
        # lat_vis = lat_vis.view(-1, self.nps+1, lat_vis.shape[-1])

        topo_loss_list = []    
        for i in range(self.nps):
            loss_topo_ne, loss_topo_po = self.LossManifold(
                latent_data=lat_vis_list[0],
                latent_data_aug=lat_vis_list[i+1],
                sigma=self.hparams.sigma,
                w_nb=self.hparams.w_nb,
                w_fp=self.hparams.w_fp
            )
            loss_topo_item = (loss_topo_ne*self.hparams.weight_nepo + loss_topo_po)/(1+self.hparams.weight_nepo) # + loss_crossentropy
            topo_loss_list.append(loss_topo_item)
            
        
        loss_topo = torch.stack(topo_loss_list).mean()
        loss_mse = ((self.dec(lat_high_dim_list[0])-data_feature)**2).mean()
    ...
    def validation_step(self, batch, batch_idx, test=False):
        ...
        self.vis_dictionary[index] = lat_vis
