...
if not os.path.exists(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/matrixplot.pdf'):
    ...
    plt.savefig(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/matrixplot.pdf', bbox_inches = 'tight', dpi = 600)
    plt.close()
if not os.path.exists(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/dotplot.pdf'):
    sc.pl.dotplot(adata, 
                     var, 
                     'dmt_leiden_anno', 
                     standard_scale = 'var',
    ...
    plt.savefig(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/dotplot.pdf', bbox_inches = 'tight', dpi = 600)
    plt.close()
...
colormap = {'Cere_sc_22': '#b720f0',
 ...
 'Cere_sc_6': '#f216bd',
 'Cere_sc_11': '#8945d4'}
if not os.path.exists(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/dmt_plot.png'):
    ...
    plt.savefig(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/dmt_plot.png', bbox_inches = 'tight', dpi = 600)
    plt.close()
...
if not os.path.exists(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/spatial_plot_all.png'):
    ...
    for name in names:
        ...
        count += 1
    plt.savefig(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/spatial_plot_all.png', bbox_inches = 'tight', dpi = 600)
    plt.close()


for i in set(adata.obs['dmt_leiden_anno']):
    if not os.path.exists(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/celltype_spatial_plot/{i}.png'):
        fig = plt.figure(figsize=(64, 24))
        x_min, x_max, y_min, y_max = float('inf'), float('-inf'), float('inf'), float('-inf')
        for name in names:
            adata_temp = adata[adata.obs['slice_code'] == name].copy()
        ...
        plt.savefig(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/celltype_spatial_plot/{i}.png', bbox_inches = 'tight', dpi = 600)
        plt.close()

for gene in var:
    try:
        if not os.path.exists(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/gene_plot/{gene}.pdf'):
            fig = plt.figure(figsize=(64, 24))
            gs = GridSpec(4, 8, figure=fig)
            count = 0
            for name in names:
                ...
                if count == 0:
                    ...
                    ax.add_artist(scalebar)
                count += 1
            plt.savefig(f'/data/work/05.cluster/FuseMap/20251103/2_cerebellum_single/gene_plot/{gene}.png', bbox_inches = 'tight', dpi = 600)
            plt.close()
    except:
        continue